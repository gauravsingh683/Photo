...
Write-Host "Fixing camera preview aspect ratio..."
...
const App: React.FC = () => {
  ...
  return (
    <div style={{ backgroundColor: '#000', color: '#fff', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: '"Inter", sans-serif', position: 'relative', overflow: 'hidden' }}>
      ...
      {/* FIXED: Wrapped the camera and preview screens in a fixed aspect ratio container that perfectly matches the final frame output to prevent stretching and bad cropping */}
      {(appState === 'FILTER_SELECT' || appState === 'COUNTDOWN') && (
        <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          
          <div style={{ position: 'relative', height: '100%', maxWidth: '100vw', aspectRatio: '3/4', overflow: 'hidden', backgroundColor: '#111', boxShadow: '0 0 50px rgba(0,0,0,0.8)' }}>
            <Webcam 
              audio={false} 
              ref={webcamRef} 
      ...
      {/* FIXED: Preview screen now matches the 3/4 aspect ratio portrait box */}
      {appState === 'PREVIEW' && (
        <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'fadeIn 0.3s' }}>
          <div style={{ position: 'relative', height: '100%', maxWidth: '100vw', aspectRatio: '3/4', backgroundColor: '#000', overflow: 'hidden', boxShadow: '0 0 50px rgba(0,0,0,0.8)' }}>
