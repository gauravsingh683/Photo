...
Write-Host "Updating App.tsx with realistic social frames and fixing image offset bug..."
...
const App: React.FC = () => {
  ...
  const resetSession = () => {
    ...
    setAppState('FILTER_SELECT'); // Jump straight back to filter select
  };
  ...
  const FrameInsta = () => (
    <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', zIndex: 5, backgroundColor: 'white' }}>
      ...
      {/* Transparent Center where the photo shows through */}
  ...
  const FrameMagazine = () => (
    <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none', border: '15px solid white', boxSizing: 'border-box', zIndex: 5, display: 'flex', flexDirection: 'column' }}>
       {/* Ensure base images correctly fill the magazine frame as well */}
...
Write-Host "App.tsx Update Complete!"
