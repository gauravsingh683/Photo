...
function App() {
  ...
  const handleWhatsAppClick = async () => {
    ...
    try {
      ...
      if (el) {
        ...
        const res = await fetch('http://localhost:5173/api/upload', { method: 'POST', body: JSON.stringify({ image: base64Image }) });
  ...
  const handlePrint = async () => {
    try {
      ...
      const canvas = await html2canvas(el, { useCORS: true, scale: 3 });
      const imgData = canvas.toDataURL('image/jpeg', 1.0);
      
      // Determine CSS page size
      let cssSize = 'auto';
      if (printSize === '4x6') cssSize = '4in 6in';
      else if (printSize === '2x6') cssSize = '2in 6in';
      else if (printSize === '6x4') cssSize = '6in 4in';
    ...
    if (effectiveLayout === 'strip3') {
      return (
        <div style={Object.assign({ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'grid', gridTemplateColumns: '1fr 1fr', gridTemplateRows: '1fr 1fr', gap: '15px', padding: '20px' }, f.imageStyle)}>
          {renderImages.slice(0, 3).map((src, i) => (
             <img key={i} src={src} style={{ width: '100%', height: '100%', objectFit: 'cover', transform: src.includes('placeholder') ? 'none' : 'scaleX(-1)', filter: filterCss, borderRadius: '8px', gridColumn: i === 2 ? '1 / span 2' : 'auto' }} />
          ))}
        </div>
      );
      ...
      return (
        <div style={Object.assign({ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'grid', gridTemplateColumns: '1fr 1fr', gridTemplateRows: '1fr 1fr', gap: '15px', padding: '20px' }, f.imageStyle)}>
          {renderImages.slice(0, 4).map((src, i) => (
             <img key={i} src={src} style={{ width: '100%', height: '100%', objectFit: 'cover', transform: src.includes('placeholder') ? 'none' : 'scaleX(-1)', filter: filterCss, borderRadius: '8px' }} />
          ))}
        </div>
      );
    } else {
      return (
        <div style={Object.assign({ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0 }, f.imageStyle)}>
          <img src={renderImages[0] || ''} style={{ width: '100%', height: '100%', objectFit: 'cover', transform: renderImages[0].includes('placeholder') ? 'none' : 'scaleX(-1)', filter: filterCss }} />
        </div>
      );
    }
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', backgroundColor: '#ffffff', color: '#4a4a4a', fontFamily: 'sans-serif', position: 'relative', overflow: 'hidden' }}>
      ...
      {(appState === 'CAMERA' || appState === 'COUNTDOWN') && (
        <div style={{ width: '100%', height: '100vh', position: 'relative', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          <div style={{ position: 'relative', width: '100%', maxWidth: '600px', height: '100%', overflow: 'hidden', boxShadow: '0 20px 50px rgba(0,0,0,0.15)' }}>
  ...
  return (
      ...
      {appState === 'FINAL' && imagesSrc.length > 0 && (
        <div style={{ width: '100%', height: '100vh', display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: '0', backgroundColor: '#fdfbf7', overflow: 'hidden', gap: '0px' }}>
          
          {/* Outer Left Column (Custom Frames) */}
          {captureMode === 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', height: '600px', zIndex: 10, marginRight: '10px' }}>
             <div style={{ padding: '8px 20px', border: '2px solid #ff4757', borderRadius: '30px', color: '#ff4757', fontWeight: 'bold', marginBottom: '15px', textTransform: 'uppercase', letterSpacing: '1px', fontSize: '0.85rem' }}>
                Custom frame
             </div>
             <div className="no-scrollbar" style={{ width: '160px', flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '20px', padding: '10px', backgroundColor: 'transparent' }}>
          ...
          {/* Left Frame Column (Standard Frames) */}
          {/* We align the sidebar exactly to the height of the selfie container by using a 600px height container */}
          <div className="no-scrollbar" style={{ width: '160px', height: '600px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '20px', padding: '10px', backgroundColor: 'transparent', zIndex: 10 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', position: 'relative', margin: '0 40px', width: '450px' }}>
            <h2 style={{ fontSize: '2.5rem', marginBottom: '20px', fontWeight: 'bold', color: '#333', whiteSpace: 'nowrap' }}>Select Your Frame</h2>
            
            <div id="final-capture-container" style={{ width: '450px', height: '600px', backgroundColor: '#fff', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 20px 50px rgba(0,0,0,0.2)', position: 'relative' }}>
          ...
          {/* AI Frame Column (Far Right) */}
          {captureMode === 1 && customFrames.filter(f => f.type === 'ai').length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', height: '600px', zIndex: 10 }}>
              <div style={{ textAlign: 'center', marginBottom: '10px', color: '#ff4757', fontWeight: 'bold', fontSize: '0.8rem', border: '1px solid #ff4757', padding: '5px 10px', borderRadius: '20px', backgroundColor: '#fff', boxShadow: '0 4px 10px rgba(255, 71, 87, 0.2)' }}>
                AI IMAGE
              </div>
              <div className="no-scrollbar" style={{ width: '160px', flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '20px', padding: '10px', backgroundColor: 'transparent' }}>
